...
public class ProcedureOld {
    ...
    private String searchResultHref(Document html) throws ExecutionException, InterruptedException {
        RunnableFuture<String> e = new FutureTask<String>(new Callable<String>() {
            @Override
    ...
    private ArrayList<String> jobs(Document html) throws ExecutionException, InterruptedException {
        RunnableFuture<ArrayList<String>> e = new FutureTask<ArrayList<String>>(new Callable<ArrayList<String>>() {
            @Override
    ...
    private Jobs personProfileContent(Document profile) throws ExecutionException, InterruptedException {
        RunnableFuture<Jobs> f = new FutureTask<Jobs>(new Callable<Jobs>() {
            @Override
    ...
    private Jobs filmProfileContent(Document profile) throws ExecutionException, InterruptedException {
        RunnableFuture<List<JobEntry>> f = new FutureTask<List<JobEntry>>(new Callable<List<JobEntry>>() {
            @Override
        ...
        new Thread(f).start();
        return new Jobs(parameter, type, link, f.get());
     ...
     * 2. Job title (e.g: The Wolf of Wall Street, Star Wars, The Dark Knight.. )
